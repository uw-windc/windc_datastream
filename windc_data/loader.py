...
class WindcEnvironment:
    ...
    def gdx_data(self):
        if self.version == "windc_2_0_1":
            ...
            # regions
            regions = copy.deepcopy(self.windc_notation["region.abbv"])
            regions = regions - {"US"}
            data["r"] = {
                "type": "set",
                "elements": regions,
            ...
            data["yr"] = {
                "type": "set",
                "elements": self.windc_notation["year"],
            ...
            build_cfs_ma = dict(
                zip(
                    zip(
                        self.data["cfs_ma"]["ORIG_MA"],
                        self.data["cfs_ma"]["DEST_MA"],
                        self.data["cfs_ma"]["NAICS"],
                        self.data["cfs_ma"]["SCTG"],
                        self.data["cfs_ma"]["units"],
                    ),
                    self.data["cfs_ma"]["TOTAL_VALUE"],
                )
            )
            ...
            build_cfs_st = dict(
                zip(
                    zip(
                        self.data["cfs_st"]["ORIG_STATE"],
                        self.data["cfs_st"]["DEST_STATE"],
                        self.data["cfs_st"]["NAICS"],
                        self.data["cfs_st"]["SCTG"],
                        self.data["cfs_st"]["units"],
                    ),
                    self.data["cfs_st"]["TOTAL_VALUE"],
                )
            )
            ...
            build_co2 = dict(
                zip(
                    zip(
                        self.data["emission_rate"]["fuel"],
                        self.data["emission_rate"]["units"],
                    ),
                    self.data["emission_rate"]["value"],
                )
            )
            ...
            build_oilprice = dict(
                zip(
                    zip(
                        self.data["eia_crude_price"]["year"],
                        self.data["eia_crude_price"]["units"],
                    ),
                    self.data["eia_crude_price"]["price"],
                )
            )
            ...
            build_emiss = dict(
                zip(
                    zip(
                        self.data["eia_emissions"]["gams.sector"],
                        self.data["eia_emissions"]["State"],
                        self.data["eia_emissions"]["year"],
                        self.data["eia_emissions"]["units"],
                    ),
                    self.data["eia_emissions"]["emissions"],
                )
            )
            ...
            # gsp_units
            build_gsp = self.data["bea_gsp"].dropna(axis="index", how="any")
            build_gsp = dict(
                zip(
                    zip(
                        build_gsp["GeoName"],
            ...
            build_hr = dict(
                zip(
                    zip(
                        self.data["eia_heatrate"]["year"],
                        self.data["eia_heatrate"]["gams.variable"],
                        self.data["eia_heatrate"]["units"],
                    ),
                    self.data["eia_heatrate"]["value"],
                )
            )
            data["heatrate_units"] = {
                "type": "parameter",
            ...
            # pce_units
            build_pce = self.data["bea_pce"].dropna(axis="index", how="any")
            build_pce = dict(
                zip(
                    zip(
                        build_pce["year"],
            ...
            build_seds = dict(
                zip(
                    zip(
                        self.data["eia_seds"]["source"],
                        self.data["eia_seds"]["sector"],
                        self.data["eia_seds"]["StateCode"],
                        self.data["eia_seds"]["Year"],
                        self.data["eia_seds"]["units"],
                    ),
                    self.data["eia_seds"]["Data"],
                )
            )
            data["seds_units"] = {
                "type": "parameter",
            ...
            build_sgf = dict(
                zip(
                    zip(
                        self.data["census_sgf"]["year"],
                        self.data["census_sgf"]["State"],
                        self.data["census_sgf"]["gams.Category"],
                        self.data["census_sgf"]["units"],
                    ),
                    self.data["census_sgf"]["value"],
                )
            )
            data["sgf_units"] = {
                "type": "parameter",
            ...
            build_exim = dict(
                zip(
                    zip(
                        self.data["state_exim"]["State"],
                        self.data["state_exim"]["NAICS"],
                        self.data["state_exim"]["Time"],
                        self.data["state_exim"]["flow"],
                        self.data["state_exim"]["units"],
                    ),
                    self.data["state_exim"]["value"],
                )
            )
            data["usatrd_units"] = {
                "type": "parameter",
            ...
            build_supply = dict(
                zip(
                    zip(
                        self.data["bea_supply"]["year"],
                        self.data["bea_supply"]["gams.IOCode"],
                        self.data["bea_supply"]["gams.Commodities/Industries"],
                        self.data["bea_supply"]["units"],
                    ),
                    self.data["bea_supply"]["value"],
                )
            )
            data["supply_units"] = {
                "type": "parameter",
            ...
            build_supply_det = dict(
                zip(
                    zip(
                        self.data["bea_supply_det"]["year"],
                        self.data["bea_supply_det"]["gams.IOCode"],
                        self.data["bea_supply_det"]["gams.Commodities/Industries"],
                        self.data["bea_supply_det"]["units"],
                    ),
                    self.data["bea_supply_det"]["value"],
                )
            )
            ...
            build_use = dict(
                zip(
                    zip(
                        self.data["bea_use"]["year"],
                        self.data["bea_use"]["gams.IOCode"],
                        self.data["bea_use"]["gams.Commodities/Industries"],
                        self.data["bea_use"]["units"],
                    ),
                    self.data["bea_use"]["value"],
                )
            )
            data["use_units"] = {
                "type": "parameter",
            ...
            build_use_det = dict(
                zip(
                    zip(
                        self.data["bea_use_det"]["year"],
                        self.data["bea_use_det"]["gams.IOCode"],
                        self.data["bea_use_det"]["gams.Commodities/Industries"],
                        self.data["bea_use_det"]["units"],
                    ),
                    self.data["bea_use_det"]["value"],
                )
            )
            data["use_det_units"] = {
                "type": "parameter",
