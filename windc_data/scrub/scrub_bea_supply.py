...
def scrub_supply(from_sql):
    if from_sql == True:
        print('Sourcing BEA supply data from SQL database...')
        # query sql database
        sql = 'select * from bea_supply;'
        supply = read_sql(sql, con=windc_engine, index_col='index')
    else:
        print('Sourcing BEA supply data from raw files...')
        supply = parse_bea_supply_io.file_parser()
...
def scrub_supply_to_csv(from_sql):
    ...
    s.to_csv('bea_supply_io_scrubed.csv')
