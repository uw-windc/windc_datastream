...
def scrub_use(from_sql):
    if from_sql == True:
        print('Sourcing BEA use data from SQL database...')
        # query sql database
        sql = 'select * from bea_use;'
        use = read_sql(sql, con=windc_engine, index_col='index')
    else:
        print('Sourcing BEA use data from raw files...')
        use = parse_bea_use_io.file_parser()

    if len(use['units'].unique()) != 1:
        raise Exception(
            '... multiple units in BEA use matirx detected, make conversions were done properly')
...
def scrub_use_to_csv(from_sql):
    ...
    s.to_csv('bea_use_io_scrubed.csv')
